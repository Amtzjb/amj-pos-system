...
export const Sidebar = ({ currentView, onNavigate, onLogout, user }: Props) => {
    ...
    // Función wrapper para navegar y cerrar el menú en móvil automáticamente
    ...
    return (
        <>
            ...
            {/* Aparece flotando arriba a la izquierda cuando el menú está cerrado */}
            {!isOpen && (
                <button 
                    onClick={() => setIsOpen(true)}
                    className="md:hidden fixed top-4 left-4 z-50 bg-white p-2 rounded-lg shadow-lg border border-gray-200 text-gray-700"
                >
            ...
            {/* --- 3. EL SIDEBAR --- */}
            <div className={`
                bg-gray-100 h-screen flex flex-col p-4 border-r border-gray-200 shrink-0
